...
export const getUserEnrolledCourses = async (
  ...
  try {
    const enrolledCourses = await UserCourseProgressModel.query("userId")
      .eq(userId)
      .exec();
    const courseIds = enrolledCourses.map((item: any) => item.courseId);
    const courses = await CourseModel.batchGet(courseIds);
    res.json({
...
export const getUserCourseProgress = async (
  ...
  try {
    const progress = await UserCourseProgressModel.get({ userId, courseId });
    if (!progress) {
      res
        .status(404)
...
export const updateUserCourseProgress = async (
  ...
  try {
    let progress = await UserCourseProgressModel.get({ userId, courseId });
